...
function universities(graduates) {
  const sortedU = graduates.sort((a,b) => {
...
function contactInfo(array, email) {
  const contactInfo = [];

  for (let i = 0; i < array.length; i++) {
    if (array[i].first_name <= email) {
      first_name.push(array[i]);
    }
  }
  return contactInfo;
}
...
function unisWithUni(graduates, university){
  const filtered = graduates.filter((graduates) => graduates.university === university)
  return filtered
}
...
function lowCaseAnimalNames(zooAnimals) {
  ...
  return zooAnimals.map((zooAnimals)=> {
    return zooAnimals.first_name.toLowerCase();
  })
}
...
function lowPopulationAnimals(zooAnimals,population) {
  ...
  return zooAnimals.filter((zooAnimals)=> {
    return zooAnimals.population === lpopulation;})
  }
}
...
function animalpop(runners) {
  const populationTotal = 0;
  return zooAnimals.reduce((zooAnimals,sum)=>{return zooAnimals.population + sum},0);
